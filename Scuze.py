...
def adauga():
    ...
    conn.commit()
    print("Ai introdus cu succes date in DB")
    conn.close
...
def stergere():
    ...
    conn.commit
    print("Elementul tau a fost sters din DB")
    conn.close
...
def update():
    ...
    cursor.execute('UPDATE scuze SET scuza_text = ? WHERE ID = ?', (scuza_noua,id_modificat))
    conn.commit()
    print(f"Am modificate elemntul cu id-ul {id_modificat}")
    conn.close
